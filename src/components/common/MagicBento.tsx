...
export const MagicBentoCard: React.FC<MagicBentoCardProps> = ({
  ...
  const combinedClassName = `magic-bento-card ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${className}`;
  const customStyle = {
  ...
  return (
    <div
      ref={cardRef}
      className={`${combinedClassName} ${enableStars ? 'particle-container' : ''}`}
      style={customStyle}
    >
      {children}
    </div>
...
const MagicBento: React.FC<MagicBentoWrapperProps> = ({
  children,
